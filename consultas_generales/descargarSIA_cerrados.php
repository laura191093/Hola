...
//Reporte para excel
		...
		LEFT JOIN select_3 e ON e.id = s.select3 WHERE s.estatus='CERRADO'";
		...
		$titulosColumnasO = array('FOLIO', 'SEMANA', 'FECHA SOLICITUD','FECHA ENTREGA', 'REGION', 'CENTRO DE TRABAJO','ENTIDAD', 'ORGANIZACION','SOLICITANTE','ESTATUS', 'NARRACION', 'REQUERIMIENTO', 'ACCIONES / HALLAZGOS', 'CONCLUSIONES', 'RECOMENDACIONES','COMENTARIOS ADICIONALES','CAPTURO', 'ULTIMA ACTUALIZACION','FECHA DE REGISTRO');
		...
		while ($fila =mysql_fetch_array ($consulta_SIA)){
			$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue('K'.$io, utf8_encode($fila['narracion']))
					->setCellValue('L'.$io, utf8_encode($fila['requerimiento']))
					->setCellValue('M'.$io, utf8_encode($fila['acciones_hallazgos']))
					->setCellValue('N'.$io, utf8_encode($fila['conclusiones']))
					->setCellValue('O'.$io, utf8_encode($fila['recomendaciones']))
					->setCellValue('P'.$io, utf8_encode($fila['comentarios_adicionales']))
					->setCellValue('Q'.$io, utf8_encode($fila['registro']))
					->setCellValue('R'.$io, utf8_encode($fila['nombre_actualizo']))
					->setCellValue('S'.$io, utf8_encode($fila['fin_captura']));
