...
//Reporte para excel
		...
		mysql_select_db($database_conexion_usuarios, $conexion_usuarios);	
		$query_consulta_bajas_t= sprintf("SSELECT b.id, b.semana,  b.organizacion, r.opcion as region, c.opcion as select2,
...
LEFT JOIN region re ON re.id_region = r.region
LEFT JOIN select_2 c ON c.id = r.select2
LEFT JOIN select_3 e ON e.id = r.select3 WHERE s.supervisor= '%s'", $colname_Recordset1);
...
LEFT JOIN select_3 e ON e.id = o.select3 WHERE o.supervisor = '%s'",  $colname_Recordset1);
		...
		if($row_Asaltos > 0 || $row_consulta_bajas_t> 0 || $row_consulta_detenidos_t> 0 || $row_consulta_robos_T  > 0 ||  $row_consulta_re_t > 0 || $row_consulta_secciones_t > 0 || $row_Extendida> 0 || $row_CE> 0 || $row_investigaciones>0 || $row_Operativos > 0){
		...
		$objPHPExcel-> getSheet (9) -> setTitle ( 'Operativos' );
		...
		$titulosColumnasRE = array('FOLIO', 'SEMANA', 'FECHA','REGION','CENTRO DE VENTAS', 'ENTIDAD','ORGANIZACION','NOMBRE DEL VENDEDOR','RUTA','CLIENTE','AREA','REPORTE','HORARIO','CALLE','COLONIA','MUNICIPIO-DELEGACION','AVERIGUACION PREVIA','JEFE INMEDIATO','CANAL','BANDEJAS','CHAROLAS / TINAS','DOLLYS','PASCUALINEROS', 'VALOR EQUIPOS RECUPERADOS','PIEZAS DE PRODUCTO', 'VALOR PRODUCTOS RECUPERADOS' ,'TOTAL ROBADO','TOTAL DE RECUPERACION','CUMPLIO','MEDIDAS','NARRACION','SEGUIMIENTO','ESTATUS','SUPERVISOR');	
		// Se agregan los titulos del reporte
		...
		while ($filaRE =mysql_fetch_array ($consulta_re_t)){
			$objPHPExcel->setActiveSheetIndex(4)
					...
					->setCellValue('AB'.$ire, utf8_encode($filaRE['valor_recuperacion']))		
	...
	//OPERATIVOS
		...
		// Se activa la hoja para que sea la que se muestre cuando el archivo se abre
		$objPHPExcel->setActiveSheetIndex(0);	
		$objPHPExcel->removeSheetByIndex(10); 	
...
<form id="form2" name="form2" class= "reporte" method="post" action="">
  <label></label>
  <p><strong>-EXPORTAR REPORTE </strong>
