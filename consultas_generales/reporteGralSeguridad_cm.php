...
mysql_select_db($database_conexion_usuarios, $conexion_usuarios);
$query_consulta_usuarioAll = sprintf("SELECT Nombre FROM supervisores where categoria='SUPERVISOR' order by Nombre asc", $colname_consulta_usuarioAll);
...
LEFT JOIN senas_particulares s ON s.id_fk = a.id LEFT JOIN datos_vehiculo dv ON dv .id_fkv = a.id 
WHERE supervisor = '%s' ORDER BY id DESC", $colname_RecordsetTable);
...
LEFT JOIN region re ON re.id_region = r.region
LEFT JOIN select_2 c ON c.id = r.select2
LEFT JOIN select_3 e ON e.id = r.select3 WHERE supervisor= '%s' ORDER BY id DESC", $colname_RecordsetTable);
...
<form id="form2" name="form2" method="post" class= "reporte" action="descargarReporteGral_cm.php">
  <label></label>
...
<form id="form1" name="form1" method="post" class= "reporte" action="reporteGralSeguridad_cm.php">
  <label></label>
  <p><strong>-SELECIONAR SUPERVISOR: </strong>
...
?>
  ...
  <div id="contenidoAjax" class="divMenú" >
    ...
    <div align="center" class="Estilo11">
      <?php echo $row_Asaltos['supervisor']; ?>    </div>  
    <p>
...
<div id="div_Asaltos" style="display: none;">
    ...
    <table width="2628" border="1" align="center" cellspacing="1" id="Exportar_a_Excel">
      <tr>
        ...
        <td width="182"><div align="center">SEGUIMIENTO</div></td>
    ...
    <td><?php echo $row_Asaltos['semana']; ?></td>
        ...
        <td><?php echo $row_Asaltos['seguimiento']; ?></td>
